...
inline void ifThenElse(Bit *dest, const Bit *tsrc, const Bit *fsrc, int size,
                       Bit cond) {
  Bit x, a;
  int i = 0;
  while (size-- > 0) {
...
template <typename T> inline Integer::Integer(int len, T *input, int party) {
  bool *b = new bool[len];
...
template <typename T> inline Integer::Integer(T *input, int party) {
...
inline void Integer::revealBools(bool *bools, int party) const {
  ProtocolExecution::prot_exec->reveal(bools, party, (block *)bits.data(),
                                       size());
}

template <> inline uint32_t Integer::reveal<uint32_t>(int party) const {
  std::bitset<32> bs;
  bs.reset();
  bool b[size()];
...
template <> inline uint64_t Integer::reveal<uint64_t>(int party) const {
  std::bitset<64> bs;
  bs.reset();
  bool b[size()];
  ProtocolExecution::prot_exec->reveal(b, party, (block *)bits.data(), size());
...
template <> inline int32_t Integer::reveal<int32_t>(int party) const {
  return reveal<uint32_t>(party);
}

template <> inline int64_t Integer::reveal<int64_t>(int party) const {
  return reveal<uint64_t>(party);
}

template <> inline string Integer::reveal<string>(int party) const {
...
inline size_t Integer::size() const { return bits.size(); }
...
inline Integer Integer::operator+(const Integer &rhs) const {
  assert(size() == rhs.size());
  Integer res(*this);
  add_full(res.bits.data(), nullptr, bits.data(), rhs.bits.data(), nullptr,
           size());
  return res;
}
...
inline Integer Integer::operator-(const Integer &rhs) const {
  assert(size() == rhs.size());
  Integer res(*this);
  sub_full(res.bits.data(), nullptr, bits.data(), rhs.bits.data(), nullptr,
           size());
  return res;
}
