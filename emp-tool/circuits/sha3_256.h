...
int sha3_256(uint8_t *output, const T *input, const size_t length = 1) {
  ...
  if (EVP_DigestUpdate(mdctx, (const uint8_t *)input, length * sizeof(T)) !=
      1) {
    std::cerr << "Error in EVP_DigestUpdate(mdctx, (const uint8_t *) input, "
                 "length * sizeof(T))\n"
              << std::flush;
    EVP_MD_CTX_destroy(mdctx);
    return -3;
  ...
  // produce digest
  if (EVP_DigestFinal_ex(mdctx, output, &digest_len) !=
...
// Note that it does some setup at constructor time, so we can avoid doing that
// every time we hash. There is some internal state used during each hash, so
// don't try to use the same object in 2 threads simultaneously: just make
// multiple objects.
class SHA3_256_Calculator {
public:
  block blocks[1600]; // internal state used during hash. Probably easier to
                      // only allocate once.
  block zero, one;    // useful constants set at constructor time
  std::unique_ptr<BristolFashion>
      keccak_f; // ensures keccak_f is deleted when this is deleted.

  // Sets up BristolFashion circuit for calculating Keccak_f, and allocates some
  // space and constants.
  SHA3_256_Calculator() {
    zero = CircuitExecution::circ_exec->public_label(false);
    one = CircuitExecution::circ_exec->public_label(true);
    FILE *keccak_f_txt =
        fmemopen(emp_tool_circuits_files_bristol_fashion_Keccak_f_txt,
                 emp_tool_circuits_files_bristol_fashion_Keccak_f_txt_len, "r");
    keccak_f =
        std::unique_ptr<BristolFashion>(new BristolFashion(keccak_f_txt));
    fclose(keccak_f_txt);
  }

  // The Keccak circuit uses different endianness, so this calculates
  // "equivalent" addresses (in bits).
  size_t keccak_address_translator(size_t i) {
    return ((8 * (199 - (i / 8))) + (i % 8));
  }

  // Calculate the sha3_256 of a bunch of blocks (essentially concatenates the
  // arrays "inputs")
  void
  sha3_256(block output[], // we'll write 256 blocks to here
           const block *
               *inputs, // concatenate these to get the bitstring we're hashing.
           const size_t *input_sizes, // the size of each input array, please
           const size_t input_count =
               1) { // the number of arrays of blocks in "inputs".
    size_t index, i, j;
    for (i = 0; i < 1600; ++i) {
      blocks[i] = zero;
    }
    ...
    for (i = 0; i < input_count; ++i) {
      for (j = 0; j < input_sizes[i]; ++j) {
        blocks[keccak_address_translator(index)] =
            CircuitExecution::circ_exec->xor_gate(
    ...
    // I do not know why sha3 does this, but it needs to XOR the next byte with
    // 0x06.
  ...
  // Calculate the sha3_256 of a bunch of Integers (essentially concatenates the
  // Integers "inputs")
  void sha3_256(
      block output[], // we'll write 256 blocks to here as the output hash.
  ...
  void
  sha3_256(block output[], // we'll write 256 blocks to here as the output hash.
           const block
               input[], // the array of blocks to hash (each represents a bit)
  ...
  // arrays "inputs")
  void
  sha3_256(Integer *output, // we'll write 256 bits to here as the output hash.
           const block *
               *inputs, // concatenate these to get the bitstring we're hashing.
           const size_t *input_sizes, // the size of each input array, please
           const size_t input_count =
  ...
  // Integers "inputs")
  void sha3_256(
      Integer *output, // we'll write 256 bits to here as the output hash.
      const Integer inputs[], // we'll concatenate these, and hash the result.
      const size_t input_count = 1) { // the number of Integers in "inputs"
    output->bits.resize(256);
