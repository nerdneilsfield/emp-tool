...
template <typename T>
int aes_128_ctr(
    const __m128i key, const __m128i iv,
    ...
    const size_t length = 1, const uint64_t start_chunk = 0) {
  ...
  if (ciphertext_len != num_bytes) {
    std::cerr << "ciphertext length did not end up being the correct number of "
                 "bytes\n"
              << std::flush;
  }
  return 0;
...
// every time we encrypt. There is some internal state used during encryption,
// so don't try to use the same object in 2 threads simultaneously: just make
// multiple objects.
class AES_128_CTR_Calculator {
public:
  block keyiv[256]; // internal state used during encryption. Probably easier to
                    // only allocate once.
  block blind[128]; // internal state used during encryption. Probably easier to
                    // only allocate once.
  emp::Integer counter;
  std::unique_ptr<BristolFashion>
      circuit; // ensures circuit is deleted when this is deleted.

  // Sets up BristolFashion circuit for calculating aes, and allocates some
  // space and constants.
  AES_128_CTR_Calculator() {
    FILE *circuit_file =
        fmemopen(emp_tool_circuits_files_bristol_fashion_aes_128_txt,
                 emp_tool_circuits_files_bristol_fashion_aes_128_txt_len, "r");
    this->circuit =
        std::unique_ptr<BristolFashion>(new BristolFashion(circuit_file));
    fclose(circuit_file);
  ...
  int aes_128_ctr(
      const block key[], const block iv[],
      ...
      const int party = emp::PUBLIC, const uint64_t start_chunk = 0) {
    if (input == nullptr && output == nullptr) {
    ...
    if (length < 129) {
      ...
      if (input == nullptr) {
        ...
        for (size_t i = 0; i < length; ++i) {
          if (output == nullptr) {
            input[i] = CircuitExecution::circ_exec->xor_gate(
                input[i], this->blind[reverse_bytes(i)]);
          } else {
            output[i] = CircuitExecution::circ_exec->xor_gate(
                input[i], this->blind[reverse_bytes(i)]);
          }
        }
      }
      ...
      for (uint64_t i = 0; (128 * i) < length; ++i) {
        answer = this->aes_128_ctr(
            nullptr, iv, (input == nullptr) ? nullptr : &(input[128 * i]),
            (output == nullptr) ? nullptr : &(output[128 * i]),
            ((128 * (i + 1)) > length) ? (length - (128 * i)) : 128, party,
            i + start_chunk);
        if (answer != 0) {
          return answer;
        }
  ...
  int aes_128_ctr(
    ...
    for (uint64_t i = 0; (128 * i) < length; ++i) {
      ...
      answer = this->aes_128_ctr(
          nullptr, &(emp::Integer(128, &counter, party).bits[0].bit),
          (input == nullptr) ? nullptr : &(input[128 * i]),
          (output == nullptr) ? nullptr : &(output[128 * i]),
          ((128 * (i + 1)) > length) ? (length - (128 * i)) : 128, party, 0);
      if (answer != 0) {
        return answer;
      }
      ++count;
  ...
  int aes_128_ctr(
      const __m128i key, // key here is out-of-circuit information
      const __m128i iv,  // IV here is out-of-circuit information
      block input[], // if this is null, we'll just do a blind of length length
      block *output = nullptr,   // if this is null, we'll encrypt in place
      const size_t length = 128, // in blocks (so, basically bits)
    ...
    int success = emp::aes_128_ctr(key, iv, (uint8_t *)nullptr, bytes,
                                   (length + 7) / 8, start_chunk);
    if (success != 0) {
      return success;
    }
    ...
    if (input == nullptr) {
      ...
      if (output == nullptr) {
        for (size_t i = 0; i < length; ++i) {
          input[i] =
              CircuitExecution::circ_exec->xor_gate(input[i], blind[i].bit);
        }
      } else {
        for (size_t i = 0; i < length; ++i) {
          output[i] =
