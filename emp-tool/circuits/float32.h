...
class Float : Swappable<Float> {
public:
  std::array<Bit, 32> value;

  Float(Float &&in) : value(std::move(in.value)) {}

  Float(const Float &in) : value(in.value) {}
  ...
  template <typename O> O reveal(int party = PUBLIC) const;
