...
public:
  ...
  PlainProt(bool _print, string _filename)
      : print(_print), filename(_filename) {
...
inline void finalize_plain_prot() {
  PlainCircExec *cast_circ_exec =
      static_cast<PlainCircExec *>(CircuitExecution::circ_exec);
  PlainProt *cast_prot_exec =
      static_cast<PlainProt *>(ProtocolExecution::prot_exec);
  int64_t z_index = cast_circ_exec->gid++;
  cast_circ_exec->fout << 2 << " " << 1 << " " << 0 << " " << 0 << " "
                       << z_index << " XOR" << endl;
  for (auto v : cast_prot_exec->output_vec) {
    cast_circ_exec->fout << 2 << " " << 1 << " " << z_index << " " << v << " "
                         << cast_circ_exec->gid++ << " XOR" << endl;
  }
