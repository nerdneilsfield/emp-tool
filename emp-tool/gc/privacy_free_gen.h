...
template <typename T> class PrivacyFreeGen : public CircuitExecution {
public:
  block delta;
  PRP prp;
  T *io;
  ...
  bool is_public(const block &b, int party) { return false; }
  ...
  block public_label(bool b) { return constant[b]; }
  block and_gate(const block &a, const block &b) {
    block table[1];
    block res = privacy_free_garble(a, a ^ delta, b, b ^ delta, delta, table,
                                    gid++, &prp.aes);
    io->send_block(table, 1);
    return res;
  }
  block xor_gate(const block &a, const block &b) { return a ^ b; }
  block not_gate(const block &a) { return a ^ public_label(true); }
