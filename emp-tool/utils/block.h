...
inline bool getLSB(const block &x) { return (x[0] & 1) == 1; }

#ifdef __x86_64__
__attribute__((target("sse2"))) inline block makeBlock(uint64_t high,
                                                       uint64_t low) {
...
inline void xorBlocks_arr(block *res, const block *x, const block *y,
                          int nblocks) {
...
#endif
inline bool
cmpBlock(const block *x, const block *y, int nblocks) {
  const block *dest = nblocks + x;
  for (; x != dest;) {
