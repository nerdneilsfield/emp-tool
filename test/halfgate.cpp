...
int main(void) {
  ...
  for (int ii = 0; ii < 2; ++ii) {
    for (int jj = 0; jj < 2; ++jj) {
      for (int i = 0; i < 8; ++i) {
        ...
        w0 = halfgates_garble(data[0], data[0] ^ delta, data[1],
                              data[1] ^ delta, delta, table, &mi_gen);
        w1 = w0 ^ delta;
  ...
  for (int i = 0; i < 1024 * 1024 * 2; ++i) {
    prg.random_block(data, 2);
    w0 = halfgates_garble(data[0], data[0] ^ delta, data[1], data[1] ^ delta,
                          delta, table, &mi_gen);
    w1 = w0 ^ delta;
  ...
  cout << 1024 * 1024 * 128 / (time_from(start)) * 1e6 << " gates/second"
       << endl;
