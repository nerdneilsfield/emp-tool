...
void hash1(block *ret, block r, block *a, int sz) {
  ...
  for (; i < sz / HASH_BLOCK_SZ; ++i) {
    vector_inn_prdt_sum_red<HASH_BLOCK_SZ>(&tmp, a + i * HASH_BLOCK_SZ,
                                           chi + i * HASH_BLOCK_SZ);
    r2 = r2 ^ tmp;
  }
  if (sz % HASH_BLOCK_SZ != 0)
    vector_inn_prdt_sum_red(&tmp, a + i * HASH_BLOCK_SZ,
                            chi + i * HASH_BLOCK_SZ, sz % 128);
  *ret = r2 ^ tmp;
  delete[] chi;
}
...
void hash2(block *ret, block r, block *a, int sz) {
  ...
  for (; i < sz / HASH_BLOCK_SZ; ++i) {
    vector_inn_prdt_sum_red<HASH_BLOCK_SZ>(&tmp, a + i * HASH_BLOCK_SZ,
                                           coeff + i * HASH_BLOCK_SZ);
    r2 = r2 ^ tmp;
  }
  if (sz % HASH_BLOCK_SZ != 0)
    vector_inn_prdt_sum_red(&tmp, a + i * HASH_BLOCK_SZ,
                            coeff + i * HASH_BLOCK_SZ, sz % HASH_BLOCK_SZ);
  *ret = r2 ^ tmp;
  delete[] coeff;
}
...
int main() {
  ...
  cout << bench_size * 2 / (time_from(start)) * 1e6 << " operations/second"
       << endl;
  ...
  cout << "Benchmark: PRG+MUL: " << bench_size / (time_from(start)) * 1e6
       << " operations/second" << endl;
  ...
  cout << "Benchmark: PRG+Coefficient generation: "
       << bench_size / (time_from(start)) * 1e6 << " operations/second" << endl;
  ...
  cout << "Benchmark: PRG+packing: " << bench_size / (time_from(start)) * 1e6
       << " operations/second" << endl;
  ...
  cout << "Benchmark: PRG+XOR: " << bench_size / (time_from(start)) * 1e6
       << " operations/second" << endl;
  ...
  cout << "Benchmark: PRG+XOR: " << 1024 / (time_from(start)) * 1e6
       << " operations/second" << endl;
