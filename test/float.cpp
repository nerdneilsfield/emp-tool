...
vector<std::string> test_str{"sqr",  "sqrt", "sin", "cos",
                             "exp2", "exp",  "ln",  "log2"};
  ...
  for (int i = 0; i < runs; ++i) {
    ...
    if (precision > 0.0) {
      if (not accurate(res.reveal<double>(PUBLIC), Op()(da, db), precision)) {
        cout << "Inaccuracy:\t" << typeid(Op2).name() << "\t" << da << "\t"
             << db << "\t" << Op()(da, db) << "\t" << res.reveal<double>(PUBLIC)
             << endl;
      }
    } else {
...
void test_float(int func_id, double precision, double minimize,
                int runs = 1000) {
  PRG prg;
  int rate_cnt = 0;
  ...
  for (int i = 0; i < runs; ++i) {
    ...
    if (precision == 0.0) {
      if (not equal(res, comp)) {
        cout << "Inaccuracy:\t" << da << "\t"
             << "\t" << comp << "\t" << res.reveal<double>(PUBLIC) << endl;
        rate_cnt++;
      }
    } else {
  ...
  cout << "function " << test_str[func_id] << "\t\t\tDONE"
       << "  -  accuracy : " << (1.0 - (float)rate_cnt / runs) << endl;
}
