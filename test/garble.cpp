...
int port, party;
template <typename T> void test(T *netio) {
  ...
  if (party == BOB) {
    ...
    cout << "Pure AES garbling speed : " << 100 * 6800 / interval
    ...
    cout << "AES garbling + Writing to Memory : " << 100 * 6800 / interval
         << " million gate per second\n";
    delete mio;
    ...
    interval = time_from(start);
    cout << "AES garbling + Loopback Network : " << 100 * 6800 / interval
         << " million gate per second\n";
...
int main(int argc, char **argv) {
  ...
  HighSpeedNetIO *hsnetio = new HighSpeedNetIO(
      party == ALICE ? nullptr : "127.0.0.1", port, port + 1);
